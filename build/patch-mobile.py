...
@media (min-width:769px){
  ...
  /* 上移 30px：视觉重心比几何中心高一点才不显得坠（吉吉 2026-09-07「视觉点有点靠下」）。
     ...
     会把顶部推到可视区外面、还滚不回去。 */
  .wrap{margin:auto;padding:40px 32px 100px;min-height:var(--home-min-h)}
  /* 🔴 min-height 是为了让三个角色的标题停在同一个高度。
     上面那条 margin:auto 的居中（吉吉 2026-09-07 要的）默认首页内容高度是固定的，
     加了角色层之后不成立了：设计那一屏 970 高（超过视口、贴顶排），
     产品和前端只有 650，富余 250 被上下均分 → 标题往下掉 125px，
     切一次角色标题弹一次，看着像页面没加载好。
     把三个角色垫到同一高度，居中规则照旧生效，只是三边同时生效、同时不生效。
     🔴 970 是量出来的、不是拍的，设计那一屏内容一改它就不准了 ——
     所以 _verify/roles.js 有一条「三个角色 h1 距顶必须相等」的断言兜着，
     那条红了就回来重量这个值，别去调阈值。 */
}
'''
