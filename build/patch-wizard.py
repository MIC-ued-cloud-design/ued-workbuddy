...
const WIZ = {
  ...
  'research/0': {
    ...
    steps:[
      { t:'要回答什么', h:'先有问题再去比。没有问题的对照表，写完没人用',
        qs:[
      ...
      { t:'怎么取证', h:'截图估不出数值。要么进真页面量，要么就别写精确值',
        qs:[
      ...
      { t:'结论要长成什么样', h:'「可借鉴」和「不能照搬」要分开写，后者更值钱',
        qs:[
  ...
  'research/2': {
    ...
    steps:[
      ...
      { t:'翻成设计目标', h:'G那一列写的是「界面上要达成什么」，不是「业务上要涨什么」',
        qs:[
  ...
  'demo/0': {
    ...
    out:['双击就能打开的单文件网页','各断点逐档对齐（1024单独核过）',
         '状态齐全（不只有成功态）','五道门跑过（几何／覆盖／视觉／状态重排／脱网）',
         '说明里标出哪些是我推的、哪些是占位'],
    skills:[['figma-to-html-feique','1:1还原闭环'],
  ...
  'demo/2': {
    ...
    steps:[
      { t:'哪一端', h:'这一问必须先答。两套库的key一个都不通用，用错了稿子导不进去',
        qs:[
  ...
  'deliver/0': {
    ...
    steps:[
      ...
      { t:'查哪几类', h:'挑相关的认真查，不相关的标出来说明为什么不查',
        qs:[
  ...
  'deliver/1': {
    ...
    steps:[
      { t:'比哪两个', h:'两边都要能回溯，否则差异清单过两周就没人认得',
        qs:[
  ...
  'review/0': {
    ...
    steps:[
      ...
      { t:'走哪条路', h:'一条完整路径，从进到出。别东点一下西点一下',
        qs:[
      ...
      { t:'找什么', h:'找的是「他会停下来的地方」，不是「不好看的地方」',
        qs:[
      ...
      { t:'结论要落成什么', h:'「体验不好」不是结论，「他在这一步不知道下一步点哪」才是',
        qs:[
  ...
  'review/1': {
    ...
    steps:[
      ...
      { t:'要回答什么', h:'行为数据只能回答「他们做了什么」，回答不了「为什么」',
        qs:[
      ...
      { t:'样本够不够', h:'先读采样率，再定接口。顺序反了，后面的数都不知道代表什么',
        qs:[
