...
(function(){
  ...
  var SYS = [
    ...
    '      但回答开头要写一句「这是通用知识，不是 MIC 的规定」，让人知道这句话没有文档背书。',
    '   分不清一个问题属于哪一类时，按 ① 办：宁可说资料里没有，也不要编 MIC 的规则。',
  ...
  function ctxOf(chunks){
    /* 零命中的兜底话术必须跟规则 1 的两类分层一致 ——
       原来这里写「不要凭常识回答」，会把「今天几号」这类通用问题也一起挡掉。 */
    if(!chunks.length) return '（这次没检索到相关资料。如果这是 MIC 业务或飞鹊规范的问题，'
      + '就如实告诉用户资料库里没有这一条，不要凭印象编；如果这是跟 MIC 无关的通用问题，'
      + '就按规则 1 的第二类直接回答，并说明这是通用知识。）';
