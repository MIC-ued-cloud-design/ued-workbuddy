...
function ensureWorkspaceGuide() {
  try {
    ...
    const txt = [GUIDE_MARK, '# 在这个工作区里干活的约定', '',
      ...
      '- `docs/飞鹊Web组件库.css` —— 飞鹊组件的**网页实现**（`.btn` `.inp` `.alert` `.tag`……423 个类）。做网页时组件规则从这里原样复制进单文件，**不自己写一套按钮 / 输入框 / 提示条**，也不发明飞鹊没有的变体。用法铁律在 `docs/飞鹊组件使用经验和规范.md` 第零、一节。',
...
function systemPrompt(meta) {
  return [
    ...
    `【开工第一步·组件先进 <style>】做任何网页产物，先把${PACK_DIR}/docs/飞鹊组件-网页速用.css整段内联进index.html的 <style>（一条cat就行，十几KB），然后按钮 / 输入框 / 提示条 / 标签只用里面的类名。这一步不做，后面写的按钮就都不是飞鹊的——2026-09-10两次实测都是这么翻车的。`,
    `【做网页时组件不是自己写的】飞鹊组件在网页里的实现是${PACK_DIR}/docs/飞鹊Web组件库.css（按钮 .btn＋.btn-md/.btn-lg/.btn-sm＋.btn-primary/.btn-secondary/.btn-secondary-blue/.btn-secondary-dark/.btn-text/.btn-link，输入框 .inp，提示条 .alert，标签 .tag，分页 .pg，骨架 .skel……423个类）。用到哪个组件就把它那几条规则原样复制进单文件，类名保持飞鹊的。🔴 不许自己另写一套 .btn/.inp/.alert，不许借飞鹊前缀发明 .btn-neutral这种飞鹊没有的变体；组件的高度、圆角、描边、字号一律以这份CSS为准，DESIGN.md只管颜色和排版口径。用法铁律读${PACK_DIR}/docs/飞鹊组件使用经验和规范.md第零、一节：参考页面只学布局，实现只能用飞鹊的砖。`,
